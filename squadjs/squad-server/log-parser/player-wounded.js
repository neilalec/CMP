...
export default {
  regex:
    /^\[([0-9.:-]+)]\[([ 0-9]*)]LogSquadTrace: \[DedicatedServer](?:ASQSoldier::)?Wound\(\): Player:(.+?) KillingDamage=(-?[0-9.]+) from (.+?) \(Online IDs:\s*([^|)]+)\s*\|\s*(?:Cont(?:r)?oller|Player Controller) ID:\s*([^) ]+)\)\s*caused by ([A-Za-z_0-9-]+)_C/,
