...
Direct exercise of the **official in-browser mock** returned 13 synthetic players with Steam IDs, accepted a faction change, changed `status.map` immediately on `changeMap`, and returned messages for map/end operations. It supplied numeric `matchSeconds` and `scoreCap=100`. This independently confirms the console mock's **high-level client behavior**, not the raw HTTP transport or a live server. Its immediate map change directly differs from Warcon's reported live delayed travel, and its timing fields differ from the reported live status. The local HTTP fixture deliberately queues map change to exercise the more cautious observation flow.
