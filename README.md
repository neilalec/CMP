...
I built this as a larger portfolio project to practise real-time application design, backend state management, deployment hardening and testable service code. It began as a university honours project and has continued as a community tool used for organised competitive matches, with around 50 active users in a typical week. It is not intended to be a commercial matchmaking service, but it does show the shape of a production-style web app with persistent state, admin controls and external game-server integration.
